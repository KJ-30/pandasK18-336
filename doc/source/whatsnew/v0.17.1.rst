...
- ``pd.read_*`` functions can now also accept :class:`python:pathlib.Path`, or ``py.path.local``
...
- Improve the error message in ``pandas.io.gbq.to_gbq`` when a streaming insert fails (:issue:`11285`)
